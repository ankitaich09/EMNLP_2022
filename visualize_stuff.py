...
def bipolar_viz():
    path = '/Users/ankit/Documents/SSPA Project/JSON/Bipolar Disorder_scene2/'
    ...
    for file in all_files:
        with open(file) as fp:
            data = json.load(fp)
            temp_pos = data['Feature_set_1']['positive_score']
            temp_neg = data['Feature_set_1']['negative_score']
            list_of_neg.append(temp_neg)
            list_of_pos.append(temp_pos)
    ...
    plt.figure()
    plt.plot(list_of_pos, 'go', list_of_neg, 'ro')
    plt.xlabel('Participant Number')
    plt.ylabel('Score green in positive red in negative')
    plt.xticks(np.arange(0, 50, 5.0))
    plt.title('negative and positive dist')
