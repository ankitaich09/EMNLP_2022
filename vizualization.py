...
path_scene_2 = '/Users/ankit/Documents/SSPA Project/Datasets/all_features_300_scene_1.csv'
...
def violin_plot(feature):
    ...
    ax = sn.violinplot(x=x_axis, y=y_axis)
    temp = feature + ' by class'
    ax.set_title(temp)
...
def viz_corr():
    features = []
    while True:
        f_name = input('Enter feature you want: ')
        features.append(f_name)
        next = input('Wanna enter more features? y/n : ')
        if next == 'n':
            break

    features = [i.strip("'") for i in features]

    bpd, schizo, hc = correlation_matrix_plot(features)

    plt.figure()
    sn.heatmap(bpd, annot=True)
    plt.title('Correlation of features for people with bipolar')
    plt.figure()
    sn.heatmap(schizo, annot=True)
    plt.title('Correlation of features for people with Schizophrenia')
    plt.figure()
    sn.heatmap(hc, annot=True)
    plt.title('Correlation of features for people who are Healthy')
    plt.show()
